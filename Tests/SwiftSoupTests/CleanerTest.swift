...
class CleanerTest: XCTestCase {
    ...
    func testHandlesCustomProtocols()throws {
        ...
        //        let dropped = try Parser.cleanBodyFragment(html, Whitelist.basicWithImages())
        //        XCTAssertEqual("<img> \n<img>", dropped)

        let preserved = try Parser.cleanBodyFragment(html, whitelist: Whitelist.basicWithImages().addProtocols("img", "src", "cid", "data"))
    ...
    func testCleanAnchorProtocol()throws {
        ...
        var cleanHtml = try Parser.cleanBodyFragment(validAnchor, whitelist: Whitelist.relaxed())
        XCTAssertEqual("<a>Valid anchor</a>", cleanHtml)

        cleanHtml = try Parser.cleanBodyFragment(invalidAnchor, whitelist: Whitelist.relaxed())
    ...
    func testIsValid()throws {
        ...
        XCTAssertTrue(try Parser.validateBodyFragment(ok, whitelist: Whitelist.basic()))
        XCTAssertFalse(try Parser.validateBodyFragment(nok1, whitelist: Whitelist.basic()))
        XCTAssertFalse(try Parser.validateBodyFragment(nok2, whitelist: Whitelist.basic()))
        XCTAssertFalse(try Parser.validateBodyFragment(nok3, whitelist: Whitelist.basic()))
    }

    func testResolvesRelativeLinks()throws {
        let html = "<a href='/foo'>Link</a><img src='/bar'>"
        let clean = try Parser.cleanBodyFragment(html, baseURI: "http://example.com/", whitelist: Whitelist.basicWithImages())
    ...
    func testPreservesRelativeLinksIfConfigured()throws {
        ...
        let clean = try Parser.cleanBodyFragment(html, baseURI: "http://example.com/", whitelist: Whitelist.basicWithImages().preserveRelativeLinks(true))
        XCTAssertEqual("<a href=\"/foo\" rel=\"nofollow\">Link</a>\n<img src=\"/bar\"> \n<img>", clean)
    }

    func testDropsUnresolvableRelativeLinks()throws {
        let html = "<a href='/foo'>Link</a>"
        let clean = try Parser.cleanBodyFragment(html, whitelist: Whitelist.basic())
    ...
    //        let customOut = try Parser.cleanBodyFragment(html, "http://foo.com/", Whitelist.relaxed(), os)
    //        let defaultOut = try Parser.cleanBodyFragment(html, "http://foo.com/", Whitelist.relaxed())
    ...
    //        let customOut2 = try Parser.cleanBodyFragment(html, "http://foo.com/", Whitelist.relaxed(), os)
    ...
    func testHandlesFramesets()throws {
        ...
        let clean = try Parser.cleanBodyFragment(dirty, whitelist: Whitelist.basic())
        XCTAssertEqual("", clean) // nothing good can come out of that

        let dirtyDoc: Document = Parser.parseHTML(dirty)!
    ...
    func testCleanHeadAndBody() throws {
        ...
        let dirtyDoc = Parser.parseHTML(dirty)!
    ...
    func testScriptTagInWhiteList()throws {
        ...
        XCTAssertTrue( Parser.validateBodyFragment("Hello<script>alert('Doh')</script>World !", whitelist: whitelist ) )
    }
...
//                _ = try doc.text()
//                _ = try doc.outerHtml()
//                _ =  try Parser.cleanBodyFragment(dirty, Whitelist.basic())
//            }catch{
//                
//            }
//            
