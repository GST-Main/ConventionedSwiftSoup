...
class CleanerTest: XCTestCase {
    ...
    func testHandlesCustomProtocols()throws {
        ...
        //        let dropped = try HTMLParser.cleanBodyFragment(html, Whitelist.basicWithImages())
        //        XCTAssertEqual("<img> \n<img>", dropped)

        let preserved = try HTMLParser.cleanBodyFragment(html, whitelist: Whitelist.basicWithImages().addProtocols("img", "src", "cid", "data"))
    ...
    func testSimpleBehaviourTest()throws {
        ...
        let cleanHtml = try HTMLParser.cleanBodyFragment(h, whitelist: Whitelist.simpleText())
    ...
    func testBasicBehaviourTest()throws {
        ...
        let cleanHtml = try HTMLParser.cleanBodyFragment(h, whitelist: Whitelist.basic())
    ...
    func testBasicWithImagesTest()throws {
        ...
        let cleanHtml = try HTMLParser.cleanBodyFragment(h, whitelist: Whitelist.basicWithImages())
    ...
    func testRelaxed()throws {
        ...
        let cleanHtml = try HTMLParser.cleanBodyFragment(h, whitelist: Whitelist.relaxed())
    ...
    func testRemoveTags()throws {
        ...
        let cleanHtml = try HTMLParser.cleanBodyFragment(h, whitelist: Whitelist.basic().removeTags("a"))
    ...
    func testRemoveAttributes()throws {
        ...
        let cleanHtml = try HTMLParser.cleanBodyFragment(h, whitelist: Whitelist.basic().removeAttributes("blockquote", "cite"))
    ...
    func testRemoveEnforcedAttributes()throws {
        let h = "<div><p><A HREF='HTTP://nice.com'>Nice</a></p><blockquote>Hello</blockquote>"
        let cleanHtml = try HTMLParser.cleanBodyFragment(h, whitelist: Whitelist.basic().removeEnforcedAttribute("a", "rel"))
    ...
    func testRemoveProtocols()throws {
        ...
        let cleanHtml = try HTMLParser.cleanBodyFragment(h, whitelist: Whitelist.basic().removeProtocols("a", "href", "ftp", "mailto"))
    ...
    func testCleanAnchorProtocol()throws {
        ...
        var cleanHtml = try HTMLParser.cleanBodyFragment(validAnchor, whitelist: Whitelist.relaxed())
        XCTAssertEqual("<a>Valid anchor</a>", cleanHtml)

        cleanHtml = try HTMLParser.cleanBodyFragment(invalidAnchor, whitelist: Whitelist.relaxed())
        ...
        cleanHtml = try HTMLParser.cleanBodyFragment(validAnchor, whitelist: relaxedWithAnchor)
        XCTAssertEqual(validAnchor, cleanHtml)

        // An invalid anchor is never valid.
        cleanHtml = try HTMLParser.cleanBodyFragment(invalidAnchor, whitelist: relaxedWithAnchor)
        XCTAssertEqual("<a>Invalid anchor</a>", cleanHtml)
    }
    ...
    func testIsValid()throws {
        ...
        XCTAssertTrue(try HTMLParser.validateBodyFragment(ok, whitelist: Whitelist.basic()))
        XCTAssertFalse(try HTMLParser.validateBodyFragment(nok1, whitelist: Whitelist.basic()))
        XCTAssertFalse(try HTMLParser.validateBodyFragment(nok2, whitelist: Whitelist.basic()))
        XCTAssertFalse(try HTMLParser.validateBodyFragment(nok3, whitelist: Whitelist.basic()))
    }

    func testResolvesRelativeLinks()throws {
        let html = "<a href='/foo'>Link</a><img src='/bar'>"
        let clean = try HTMLParser.cleanBodyFragment(html, baseURI: "http://example.com/", whitelist: Whitelist.basicWithImages())
    ...
    func testPreservesRelativeLinksIfConfigured()throws {
        ...
        let clean = try HTMLParser.cleanBodyFragment(html, baseURI: "http://example.com/", whitelist: Whitelist.basicWithImages().preserveRelativeLinks(true))
        XCTAssertEqual("<a href=\"/foo\" rel=\"nofollow\">Link</a>\n<img src=\"/bar\"> \n<img>", clean)
    }

    func testDropsUnresolvableRelativeLinks()throws {
        let html = "<a href='/foo'>Link</a>"
        let clean = try HTMLParser.cleanBodyFragment(html, whitelist: Whitelist.basic())
    ...
    func testHandlesAllPseudoTag()throws {
        ...
        let clean = try HTMLParser.cleanBodyFragment(html, whitelist: whitelist)
    ...
    //        let customOut = try HTMLParser.cleanBodyFragment(html, "http://foo.com/", Whitelist.relaxed(), os)
    //        let defaultOut = try HTMLParser.cleanBodyFragment(html, "http://foo.com/", Whitelist.relaxed())
    ...
    //        let customOut2 = try HTMLParser.cleanBodyFragment(html, "http://foo.com/", Whitelist.relaxed(), os)
    ...
    func testHandlesFramesets()throws {
        ...
        let dirtyDoc: Document = HTMLParser.parseHTML(dirty)!
    ...
    func testCleanHeadAndBody() throws {
        ...
        let dirtyDoc = HTMLParser.parseHTML(dirty)!
    ...
    func testCleansInternationalText()throws {
        XCTAssertEqual("привет", try HTMLParser.cleanBodyFragment("привет", whitelist: Whitelist.none()))
    }

    func testScriptTagInWhiteList()throws {
        let whitelist: Whitelist = try Whitelist.relaxed()
        try whitelist.addTags( "script" )
        XCTAssertTrue( HTMLParser.validateBodyFragment("Hello<script>alert('Doh')</script>World !", whitelist: whitelist ) )
    }
...
//                _ = try doc.text()
//                _ = try doc.outerHtml()
//                _ =  try HTMLParser.cleanBodyFragment(dirty, Whitelist.basic())
//            }catch{
//                
//            }
//            
