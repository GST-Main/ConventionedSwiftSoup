...
class NodeTest: XCTestCase {
	...
	func testSetBaseUriIsRecursive() {
        let doc: Document = HTMLParser.parseHTML("<div><p></p></div>")!
	...
	func testHandlesAbsPrefix() {
        let doc: Document = HTMLParser.parseHTML("<a href=/foo>Hello</a>", baseURI: "https://jsoup.org/")!
	...
	func testHandlesAbsOnImage() {
        let doc: Document = HTMLParser.parseHTML("<p><img src=\"/rez/osi_logo.png\" /></p>", baseURI: "https://jsoup.org/")!
	...
	func testHandlesAbsPrefixOnHasAttr() {
        ...
        let doc: Document = HTMLParser.parseHTML("<a id=1 href='/foo'>One</a> <a id=2 href='https://jsoup.org/'>Two</a>")!
	...
	func testLiteralAbsPrefix() {
        ...
        let doc: Document = HTMLParser.parseHTML("<a abs:href='odd'>One</a>")!
...
*/
	func testHandleAbsOnLocalhostFileUris() {
        let doc: Document  = HTMLParser.parseHTML("<a href='password'>One/a><a href='/var/log/messages'>Two</a>", baseURI: "file://localhost/etc/")!
	...
	func testHandlesAbsOnProtocolessAbsoluteUris() {
        let doc1: Document = HTMLParser.parseHTML("<a href='//example.net/foo'>One</a>", baseURI: "http://example.com/")!
        let doc2: Document = HTMLParser.parseHTML("<a href='//example.net/foo'>One</a>", baseURI: "https://example.com/")!
	...
	func testAbsHandlesRelativeQuery() {
        let doc: Document = HTMLParser.parseHTML("<a href='?foo'>One</a> <a href='bar.html?foo'>Two</a>", baseURI: "https://jsoup.org/path/file?bar")!
	...
	func testAbsHandlesDotFromIndex() {
        let doc: Document = HTMLParser.parseHTML("<a href='./one/two.html'>One</a>", baseURI: "http://example.com")!
        let a1: Element? = doc.select(cssQuery: "a").first
        XCTAssertEqual("http://example.com/one/two.html", a1?.absoluteURLPath(ofAttribute: "href"))
	}

	func testRemove() {
        let doc: Document = HTMLParser.parseHTML("<p>One <span>two</span> three</p>")!
	...
	func testOwnerDocument() {
        let doc: Document = HTMLParser.parseHTML("<p>Hello")!
        let p: Element? = doc.select(cssQuery: "p").first
	...
	func testBefore() {
		do {
			let doc: Document = HTMLParser.parseHTML("<p>One <b>two</b> three</p>")!
	...
	func testUnwrap() {
		do {
			let doc: Document = HTMLParser.parseHTML("<div>One <span>Two <b>Three</b></span> Four</div>")!
	...
	func testUnwrapNoChildren() {
		do {
			let doc: Document = HTMLParser.parseHTML("<div>One <span></span> Two</div>")!
			let span: Element? = doc.select(cssQuery: "span").first
			let node: Node? = try span?.unwrap()
	...
	func testTraverse() {
		do {
			let doc: Document = HTMLParser.parseHTML("<div><p>Hello</p></div><div>There</div>")!
	...
	func testChildNodesCopy() {
        let doc: Document = HTMLParser.parseHTML("<div id=1>Text 1 <p>One</p> Text 2 <p>Two<p>Three</div><div id=2>")!
	...
	func testSupportsClone() {
        let doc: Document = HTMLParser.parseHTML("<div class=foo>Text</div>")!
