...
class ElementsTest: XCTestCase {
	...
	func testFilter()throws {
		...
		let els: Elements = doc.select(cssQuery: ".headline").select(cssQuery: "p")
	...
	func testAttributes()throws {
		...
		let doc: Document = HTMLParser.parseHTML(h)!
		let withTitle: Elements = doc.select(cssQuery: "p[title]")
	...
	func testHasAttr()throws {
		let doc: Document = HTMLParser.parseHTML("<p title=foo><p title=bar><p class=foo><p class=bar>")!
	...
	func testHasAbsAttr()throws {
		let doc: Document = HTMLParser.parseHTML("<a id=1 href='/foo'>One</a> <a id=2 href='https://google.com'>Two</a>")!
		let one: Elements = doc.select(cssQuery: "#1")
		let two: Elements = doc.select(cssQuery: "#2")
		let both: Elements = doc.select(cssQuery: "a")
	...
	func testClasses()throws {
		let doc: Document = HTMLParser.parseHTML("<div><p class='mellow yellow'></p><p class='red green'></p>")!
	...
	func testHasText()throws {
		let doc: Document = HTMLParser.parseHTML("<div><p>Hello</p></div><div><p></p></div>")!
		let divs: Elements = doc.select(cssQuery: "div")
		XCTAssertTrue(divs.hasText)
		XCTAssertFalse(doc.select(cssQuery: "div + div").hasText)
	}

	func testHtml()throws {
		let doc: Document = HTMLParser.parseHTML("<div><p>Hello</p></div><div><p>There</p></div>")!
		let divs: Elements = doc.select(cssQuery: "div")
	...
	func testVal()throws {
        guard let doc = HTMLParser.parseHTML("<input value='one' /><textarea>two</textarea>") else {
	...
	func testNot()throws {
		let doc: Document = HTMLParser.parseHTML("<div id=1><p>One</p></div> <div id=2><p><span>Two</span></p></div>")!

		let div1: Elements = doc.select(cssQuery: "div").selectNot(cssQuery: ":has(p > span)")
		XCTAssertEqual(1, div1.count)
		XCTAssertEqual("1", div1.first?.id)

		let div2: Elements = doc.select(cssQuery: "div").selectNot(cssQuery: "#1")
	...
	func testForms()throws {
        let doc: Document = HTMLParser.parseHTML("<form id=1><input name=q></form><div /><form id=2><input name=f></form>")!
	...
	func testClassWithHyphen()throws {
		let doc: Document = HTMLParser.parseHTML("<p class='tab-nav'>Check</p>")!
		let els: Elements = doc.getElementsByClass("tab-nav")
		XCTAssertEqual(1, els.count)
		XCTAssertEqual("Check", els.text())
	}
    
    func testEachText()throws {
        let doc: Document = HTMLParser.parseHTML("<div><p>1<p>2<p>3<p>4<p>5<p>6</div><div><p>7<p>8<p>9<p>10<p>11<p>12<p></p></div>")!
