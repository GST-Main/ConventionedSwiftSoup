...
class SelectorTest: XCTestCase {
	...
	func testNamespacedTag()throws {
		...
		let byTag: Elements = try doc.select(cssQuery: "abc|def")
		XCTAssertEqual(2, byTag.size())
		XCTAssertEqual("1", byTag.first()?.id())
		XCTAssertEqual("2", byTag.last()?.id())

		let byAttr: Elements = try doc.select(cssQuery: ".bold")
		XCTAssertEqual(1, byAttr.size())
		XCTAssertEqual("2", byAttr.last()?.id())

		let byTagAttr: Elements = try doc.select(cssQuery: "abc|def.bold")
		XCTAssertEqual(1, byTagAttr.size())
		XCTAssertEqual("2", byTagAttr.last()?.id())

		let byContains: Elements = try doc.select(cssQuery: "abc|def:contains(e)")
	...
	func testWildcardNamespacedTag()throws {
		let doc: Document = try SwiftSoup.parse("<div><abc:def id=1>Hello</abc:def></div> <abc:def class=bold id=2>There</abc:def>")
		let byTag: Elements = try doc.select(cssQuery: "*|def")
		XCTAssertEqual(2, byTag.size())
		XCTAssertEqual("1", byTag.first()?.id())
		XCTAssertEqual("2", byTag.last()?.id())
		...
		let byTagAttr: Elements = try doc.select(cssQuery: "*|def.bold")
		XCTAssertEqual(1, byTagAttr.size())
		XCTAssertEqual("2", byTagAttr.last()?.id())

		let byContains: Elements = try doc.select(cssQuery: "*|def:contains(e)")
		XCTAssertEqual(2, byContains.size())
		XCTAssertEqual("1", byContains.first()?.id())
		XCTAssertEqual("2", byContains.last()?.id())
	}
	...
	func testByAttributeRegex()throws {
		...
		let imgs: Elements = try doc.select(cssQuery: "img[src~=(?i)\\.(png|jpe?g)]")
	...
	func testByAttributeRegexCharacterClass()throws {
		...
		let imgs: Elements = try doc.select(cssQuery: "img[src~=[o]]")
	...
	func testByAttributeRegexCombined()throws {
		...
		let els: Elements = try doc.select(cssQuery: "div table[class~=x|y]")
	...
	func testCombinedWithContains()throws {
		...
		let els: Elements = try doc.select(cssQuery: "p#1 + :contains(+)")
		XCTAssertEqual(1, els.size())
	...
	func testAllElements()throws {
		...
		let doc: Document = try SwiftSoup.parse(h)
		let allDoc: Elements = try doc.select(cssQuery: "*")
		let allUnderDiv: Elements = try doc.select(cssQuery: "div *")
	...
	func testAllWithClass()throws {
		...
		let doc: Document = try SwiftSoup.parse(h)
		let ps: Elements = try doc.select(cssQuery: "*.first")
	...
	func testGroupOr()throws {
		...
		let doc: Document = try SwiftSoup.parse(h)
		let els: Elements = try doc.select(cssQuery: "p,div,[title]")
	...
	func testDescendant()throws {
		...
		let aboveRoot: Elements = try root.select(cssQuery: "body div.head")
	...
	func testAnd()throws {
		...
		let div: Elements = try doc.select(cssQuery: "div.foo")
		XCTAssertEqual(1, div.size())
		XCTAssertEqual("div", div.first()?.tagName())

		let p: Elements = try doc.select(cssQuery: "div .foo") // space indicates like "div *.foo"
		XCTAssertEqual(1, p.size())
		XCTAssertEqual("p", p.first()?.tagName())

		let div2: Elements = try doc.select(cssQuery: "div#1.foo.bar[title=bar][name=qux]") // very specific!
		XCTAssertEqual(1, div2.size())
		XCTAssertEqual("div", div2.first()?.tagName())

		let p2: Elements = try doc.select(cssQuery: "div *.foo") // space indicates like "div *.foo"
	...
	func testParentWithClassChild()throws {
		...
		let allAs: Elements = try doc.select(cssQuery: "h1 > a")
		XCTAssertEqual(3, allAs.size())
		XCTAssertEqual("a", allAs.first()?.tagName())

		let fooAs: Elements = try doc.select(cssQuery: "h1.foo > a")
		XCTAssertEqual(2, fooAs.size())
		XCTAssertEqual("a", fooAs.first()?.tagName())

		let barAs: Elements = try doc.select(cssQuery: "h1.foo > a.bar")
	...
	func testParentChildStar()throws {
		...
		let doc: Document = try SwiftSoup.parse(h)
		let divChilds: Elements = try doc.select(cssQuery: "div > *")
	...
	func testMultiChildDescent()throws {
		...
		let doc: Document = try SwiftSoup.parse(h)
		let els: Elements = try doc.select(cssQuery: "div#foo > h1.bar > a[href*=example]")
		XCTAssertEqual(1, els.size())
	...
	func testCaseInsensitive()throws {
		...
		XCTAssertEqual(2, try doc.select(cssQuery: "DIV").size())
		XCTAssertEqual(1, try doc.select(cssQuery: "DIV[TITLE]").size())
		XCTAssertEqual(1, try doc.select(cssQuery: "DIV[TITLE=BAR]").size())
		XCTAssertEqual(0, try doc.select(cssQuery: "DIV[TITLE=BARBARELLA").size())
	}
	...
	func testNotAdjacent()throws {
		let h: String = "<ol><li id=1>One<li id=2>Two<li id=3>Three</ol>"
		let doc: Document = try SwiftSoup.parse(h)
		let sibs: Elements = try doc.select(cssQuery: "li#1 + li#3")
	...
	func testMixCombinatorGroup()throws {
		let h: String = "<div class=foo><ol><li>One<li>Two<li>Three</ol></div>"
		let doc: Document = try SwiftSoup.parse(h)
		let els: Elements = try doc.select(cssQuery: ".foo > ol, ol > li + li")
	...
	func testPseudoContains()throws {
		...
		let ps1: Elements = try doc.select(cssQuery: "p:contains(Rain)")
		XCTAssertEqual(3, ps1.size())

		let ps2: Elements = try doc.select(cssQuery: "p:contains(the rain)")
		XCTAssertEqual(2, ps2.size())
		try XCTAssertEqual("The Rain.", ps2.first()?.html())
		try XCTAssertEqual("The <i>rain</i>.", ps2.last()?.html())

		let ps3: Elements = try doc.select(cssQuery: "p:contains(the Rain):has(i)")
		XCTAssertEqual(1, ps3.size())
		try XCTAssertEqual("light", ps3.first()?.className())

		let ps4: Elements = try doc.select(cssQuery: ".light:contains(rain)")
		XCTAssertEqual(1, ps4.size())
		try XCTAssertEqual("light", ps3.first()?.className())

		let ps5: Elements = try doc.select(cssQuery: ":contains(rain)")
	...
	func testPsuedoContainsWithParentheses()throws {
		...
		let ps1: Elements = try doc.select(cssQuery: "p:contains(this (is good))")
		XCTAssertEqual(1, ps1.size())
		XCTAssertEqual("1", ps1.first()?.id())

		let ps2: Elements = try doc.select(cssQuery: "p:contains(this is bad\\))")
		XCTAssertEqual(1, ps2.size())
	...
	func testContainsOwn()throws {
		...
		let ps: Elements = try doc.select(cssQuery: "p:containsOwn(Hello now)")
		XCTAssertEqual(1, ps.size())
		XCTAssertEqual("1", ps.first()?.id())

		XCTAssertEqual(0, try doc.select(cssQuery: "p:containsOwn(there)").size())
	}
	...
	func testMatches()throws {
		...
		let p1: Elements = try doc.select(cssQuery: "p:matches(The rain)") // no match, case sensitive
		XCTAssertEqual(0, p1.size())

		let p2: Elements = try doc.select(cssQuery: "p:matches((?i)the rain)") // case insense. should include root, html, body
		XCTAssertEqual(1, p2.size())
		XCTAssertEqual("1", p2.first()?.id())

		let p4: Elements = try doc.select(cssQuery: "p:matches((?i)^rain$)") // bounding
		XCTAssertEqual(1, p4.size())
		XCTAssertEqual("4", p4.first()?.id())

		let p5: Elements = try doc.select(cssQuery: "p:matches(\\d+)")
		XCTAssertEqual(1, p5.size())
		XCTAssertEqual("2", p5.first()?.id())

		let p6: Elements = try doc.select(cssQuery: "p:matches(\\w+\\s+\\(\\w+\\))") // test bracket matching
		XCTAssertEqual(1, p6.size())
		XCTAssertEqual("3", p6.first()?.id())

		let p7: Elements = try doc.select(cssQuery: "p:matches((?i)the):has(i)") // multi
	...
	func testMatchesOwn()throws {
		...
		let p1: Elements = try doc.select(cssQuery: "p:matchesOwn((?i)hello now)")
		XCTAssertEqual(1, p1.size())
		XCTAssertEqual("1", p1.first()?.id())

		XCTAssertEqual(0, try doc.select(cssQuery: "p:matchesOwn(there)").size())
	}

	func testRelaxedTags()throws {
		let doc: Document = try SwiftSoup.parse("<abc_def id=1>Hello</abc_def> <abc-def id=2>There</abc-def>")

		let el1: Elements = try doc.select(cssQuery: "abc_def")
		XCTAssertEqual(1, el1.size())
		XCTAssertEqual("1", el1.first()?.id())

		let el2: Elements = try doc.select(cssQuery: "abc-def")
	...
	func testNotAll()throws {
		...
		let el1: Elements = try doc.body()!.select(cssQuery: ":not(p)") // should just be the span
		XCTAssertEqual(2, el1.size())
	...
	func testNotClass()throws {
		...
		let el1: Elements = try doc.select(cssQuery: "div:not(.left)")
		XCTAssertEqual(1, el1.size())
		XCTAssertEqual("1", el1.first()?.id())
	}
	...
	func testHandlesCommasInSelector()throws {
		...
		let ps: Elements = try doc.select(cssQuery: "[name=1,2]")
		XCTAssertEqual(1, ps.size())

		let containers: Elements = try doc.select(cssQuery: "div, li:matches([0-9,]+)")
	...
	func testSelectSupplementaryCharacter()throws {
			...
			XCTAssertEqual("div", try doc.select(cssQuery: "div[k" + s + "]").first()?.setTagName())
			XCTAssertEqual("div", try doc.select(cssQuery: "div:containsOwn(" + s + ")").first()?.setTagName())
		#endif
	}
