...
class SelectorTest: XCTestCase {
	...
	func testByTag()throws {
		...
		let els: Elements = HTMLParser.parseHTML("<div id=1><div id=2><p>Hello</p></div></div><DIV id=3>")!.select(cssQuery: "DIV")
		...
		let none: Elements = HTMLParser.parseHTML("<div id=1><div id=2><p>Hello</p></div></div><div id=3>")!.select(cssQuery: "span")
		XCTAssertEqual(0, none.count)
	}

	func testById()throws {
		let els: Elements = HTMLParser.parseHTML("<div><p id=foo>Hello</p><p id=foo>Foo two!</p></div>")!.select(cssQuery: "#foo")
		XCTAssertEqual(2, els.count)
		XCTAssertEqual("Hello", els.get(index: 0)!.getText())
		XCTAssertEqual("Foo two!", els.get(index: 1)!.getText())

		let none: Elements = HTMLParser.parseHTML("<div id=1></div>")!.select(cssQuery: "#foo")
		XCTAssertEqual(0, none.count)
	}

	func testByClass()throws {
		let els: Elements = HTMLParser.parseHTML("<p id=0 class='ONE two'><p id=1 class='one'><p id=2 class='two'>")!.select(cssQuery: "P.One")
		XCTAssertEqual(2, els.count)
		XCTAssertEqual("0", els.get(index: 0)!.id)
		XCTAssertEqual("1", els.get(index: 1)!.id)

		let none: Elements = HTMLParser.parseHTML("<div class='one'></div>")!.select(cssQuery: ".foo")
		XCTAssertEqual(0, none.count)

		let els2: Elements = HTMLParser.parseHTML("<div class='One-Two'></div>")!.select(cssQuery: ".one-two")
	...
	func testNamespacedTag()throws {
		let doc: Document = HTMLParser.parseHTML("<div><abc:def id=1>Hello</abc:def></div> <abc:def class=bold id=2>There</abc:def>")!
	...
	func testByAttributeStarting()throws {
		let doc: Document = HTMLParser.parseHTML("<div id=1 data-name=SwiftSoup>Hello</div><p data-val=5 id=2>There</p><p id=3>No</p>")!
	...
	func testByAttributeRegex()throws {
		let doc: Document = HTMLParser.parseHTML("<p><img src=foo.png id=1><img src=bar.jpg id=2><img src=qux.JPEG id=3><img src=old.gif><img></p>")!
	...
	func testByAttributeRegexCharacterClass()throws {
		let doc: Document = HTMLParser.parseHTML("<p><img src=foo.png id=1><img src=bar.jpg id=2><img src=qux.JPEG id=3><img src=old.gif id=4></p>")!
	...
	func testByAttributeRegexCombined()throws {
		let doc: Document = HTMLParser.parseHTML("<div><table class=x><td>Hello</td></table></div>")!
	...
	func testCombinedWithContains()throws {
		let doc: Document = HTMLParser.parseHTML("<p id=1>One</p><p>Two +</p><p>Three +</p>")!
	...
	func testGroupOrAttribute()throws {
        ...
        let els: Elements = HTMLParser.parseHTML(h)!.select(cssQuery: "[id],[title=foo]")
	...
	func testSupportsLeadingCombinator()throws {
		...
		var doc: Document = HTMLParser.parseHTML(h)!
	...
	func testPseudoCombined()throws {
		let doc: Document = HTMLParser.parseHTML("<div class='foo'><p>One</p><p>Two</p></div><div><p>Three</p><p>Four</p></div>")!
	...
	func testPseudoHas()throws {
		let doc: Document = HTMLParser.parseHTML("<div id=0><p><span>Hello</span></p></div> <div id=1><span class=foo>There</span></div> <div id=2><p>Not</p></div>")!
	...
	func testNestedHas()throws {
		let doc: Document = HTMLParser.parseHTML("<div><p><span>One</span></p></div> <div><p>Two</p></div>")!
	...
	func testPseudoContains()throws {
		let doc: Document = HTMLParser.parseHTML("<div><p>The Rain.</p> <p class=light>The <i>rain</i>.</p> <p>Rain, the.</p></div>")!
	...
	func testPsuedoContainsWithParentheses()throws {
		let doc: Document = HTMLParser.parseHTML("<div><p id=1>This (is good)</p><p id=2>This is bad)</p>")!
	...
	func testContainsOwn()throws {
		let doc: Document = HTMLParser.parseHTML("<p id=1>Hello <b>there</b> now</p>")!
	...
	func testMatches()throws {
		let doc: Document = HTMLParser.parseHTML("<p id=1>The <i>Rain</i></p> <p id=2>There are 99 bottles.</p> <p id=3>Harder (this)</p> <p id=4>Rain</p>")!
	...
	func testRelaxedTags()throws {
		let doc: Document = HTMLParser.parseHTML("<abc_def id=1>Hello</abc_def> <abc-def id=2>There</abc-def>")!
	...
	func testNotParas()throws {
		let doc: Document = HTMLParser.parseHTML("<p id=1>One</p> <p>Two</p> <p><span>Three</span></p>")!
	...
	func testNotAll()throws {
		let doc: Document = HTMLParser.parseHTML("<p>Two</p> <p><span>Three</span></p>")!
	...
	func testNotClass()throws {
		let doc: Document = HTMLParser.parseHTML("<div class=left>One</div><div class=right id=1><p>Two</p></div>")!
	...
	func testHandlesCommasInSelector()throws {
		let doc: Document = HTMLParser.parseHTML("<p name='1,2'>One</p><div>Two</div><ol><li>123</li><li>Text</li></ol>")!
	...
	func testSelectSupplementaryCharacter()throws {
			...
			let doc: Document = HTMLParser.parseHTML("<div k" + s + "='" + s + "'>^" + s + "$/div>")!
